...
(defun print-literal0 (lid &optional (out t) (ind 0))
  (format out "~a" (nspace ind))
  (print-literal lid out)
)

(defun print-clause0 (cid &optional (out t) (ind 0))
  (cond
    ((isvalid cid)
     (if (bodyof cid)
...
(defun print-proof0 (cid &optional (out t) (ind 0))
  (let ()
    (print-clause0 cid out ind)
    (cond
      ((eq (ruleof cid) :resolution)
      ...
      ((eq (ruleof cid) :REDUCED-BY-SEMANTIX)
        (let* ((pr (proofof cid))(flits (cadddr pr)))
          ...
          (loop for flid in flits do 
            (print-literal0 flid out (+ 2 ind))
            (terpri out)
            (print-proof0 (cidof flid) out (+ 2 ind)))
        )
      )
    )
  )
