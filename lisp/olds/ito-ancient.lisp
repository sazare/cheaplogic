...
(defito ito-simplesubst*()
  ...
  (intend-equal "3 cyclic sigma" '(g (g x) (h (f (g x))))
	    (subst* '((x . y)(z . (f y))(y . (g x))) '(g x (h z)))
   )

  (intend-notequal "4 cyclic sigma fail" '(g (g x) (h (f (g x))))
	    (subst* '((x . y)(y . (g x))(z . (f y))) '(g x (h z)))
   )

  (intend-equal "5 seq subst depends on the order of sigma(after)"
          '(g (f a) a)
          (subst* '((x . (f y))(y . a)) '(g x y))
  )
  (intend-notequal "6 subst depends on the order of sigma(before) never happen. but ok with unify"
          '(g (f a) a)
...
(defito ito-unify ()
  ...
  (intend-equal "3 identical fterm" '() (unify '(x) '(f x) '(f x)))
  (intend-equal "4 symbol unify" '((x . y)) (unify '(x y) 'x 'y))
  (intend-equal "5 fterm unify1" '((x . (f y))) (unify '(x y) 'x '(f y)))
  (intend-equal "6 fterm unify2" '((x . (f y))) (unify '(x y) '(f y) 'x))
  (intend-equal "7 fterm unify3" '((x . (f y))) (unify '(x y) '(h (f y)) '(h x)))
  (intend-equal "8 fterm unify4" '((x . (f y))) (unify '(x y) '(h x) '(h (f y))))
  (intend-equal "9 fterm unify5" '((w . a)(x . (f y))) (unify '(x y w) '(h x a) '(h (f y) w)))
