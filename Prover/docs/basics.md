...
Unification Theoryという、本の一生にあたるものがpdfとして公開されていて、そこにはエルブランの論文では、Unificationは規則として定義されているようなことが書かれていた。学位論文だと思うが、該当する箇所はまだみつけられていない。

Unification にまつわるいろいろなことを考えたので、書き残す。
...
このあと、代入を考え、unificationを考えるという順番。

変数はunificationを考えるときにでてくるので、代入までは変数を特別扱いしない。

### 表現の同一性
e1 = e2は、表現の定義に従って定義する。記号的な同一を≡で表すのが好みだけれど、ここは=とする。
