...
(deftest test-psubst()
  "test subst"
  (expect-equal "1 no match" 'w (psubst '() 'w))
  (expect-equal "2 no match" 'w (psubst '((y . x)(z . x)) 'w))
  (expect-equal "3 match y" 'x (psubst '((y . x)(z . x)) 'y))
  (expect-equal "4 match z" 'x (psubst '((y . x)(z . x)) 'z))

  (expect-equal "5 match y to f(x)" '(f x) (psubst '((y . (f x))(z . x)) 'y))
  (expect-equal "6 match z to f(x)" '(f x) (psubst '((y . x)(z . (f x))) 'z))
)
...
(deftest test-psubst*()
  ...
  (expect-equal "3 cyclic sigma" '(g y (h (f y)))
	    (psubst* '((x . y)(z . (f y))(y . (g x))) '(g x (h z)))
   )
  (expect-equal "4 cyclic sigma fail" '(g y (h (g x)(f y)))
	    (psubst* '((x . y)(y . (g x))(z . (f y))) '(g x (h y z)))
   )
  (expect-equal "5 seq psubst dont depends on the order of sigma(after)"
          '(g (f y) a)
          (psubst* '((x . (f y))(y . a)) '(g x y))
  )
  (expect-equal "6 psubst depends on the order of sigma(before) never happen. but ok with unify"
          '(g (f y) a)
...
(deftest test-unify ()
  "test unify on symbol"
  (expect-equal "1 identical symbolv" '() (unify '(x) 'x 'x))
  (expect-equal "2 identical symbolc" '() (unify '(x) 'a 'a))
)
(deftest test-unify-se ()
  "test unify on symbol with expression"
  (expect-equal "1 identical fterm" '() (unify '(x) '(f x) '(f x)))
  (expect-equal "2 symbol unify" '((x . y)) (unify '(x y) 'x 'y))
  (expect-equal "3 fterm unify1" '((x . (f y))) (unify '(x y) 'x '(f y)))
  (expect-equal "4 fterm unify2" '((x . (f y))) (unify '(x y) '(f y) 'x))
  (expect-equal "5 fterm unify3" '((x . (f y))) (unify '(x y) '(h (f y)) '(h x)))
  (expect-equal "6 fterm unify4" '((x . (f y))) (unify '(x y) '(h x) '(h (f y))))

  (expect-equal "7 fterm unify5" '((w . a)(x . (f y))) (unify '(x y w) '(h x a) '(h (f y) w)))
)
(deftest test-unify-inside-ok ()
  "test unify on inside ok"
  (expect-equal "1 unmatch syms" 'NO (unify '(x) 'a 'b))
  (expect-equal "2 unmatch sym-term" 'NO (unify '(x) 'a '(f x)))
  (expect-equal "3 unmatch term-sym" 'NO (unify '(x) '(f x) 'b))
  (expect-equal "4 unmatch f-g" 'NO (unify '(x y) '(f x) '(g y)))
  (expect-equal "5 unmatch f-g in h" 'NO (unify '(x y) '(h (f x)) '(h (g y))))
)

;(deftest test-unify-inside-ng ()
;
;  (expect-equal "5 fterm unify inside" '((y . (f y))) (unify '(y) 'y '(f y)))
;
;
;;;; What I should test??
